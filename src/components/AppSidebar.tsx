...
export function SidebarContent({ collapsed = false, onCollapse, isMobile = false }: SidebarContentProps) {
  ...
  const NavItem = ({ item, active }: { item: typeof NAV_SECTIONS[0]["items"][0]; active: boolean }) => {
    const linkContent = (
      ...
      <div className="flex flex-col h-full bg-sidebar text-sidebar-foreground">
        ...
        <div className="p-4 border-t border-sidebar-border space-y-1">
          ...
          <div onClick={toggleTheme}
            className={cn("w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-all duration-200 cursor-pointer", isCollapsed && "justify-center px-2")}
          >
            ...
            <Tooltip delayDuration={0}>
              <TooltipTrigger asChild>
                <button onClick={signOut}
                  className="w-full flex items-center justify-center px-2 py-2.5 rounded-lg text-sm font-medium text-red-400 hover:bg-red-500/10 transition-all duration-200 group"
                >
                  <LogOut className="w-5 h-5 shrink-0 group-hover:scale-110 transition-transform" />
                </button>
              </TooltipTrigger>
              <TooltipContent side="right" sideOffset={12} className="bg-card border-border text-destructive">
                Sair da conta
              </TooltipContent>
            </Tooltip>
          ) : (
