    ...
    <div className="p-4 lg:p-8 space-y-6">
      ...
      <div className="flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory">
        {COLUMNS.map((status) => {
          ...
          return (
            <div key={status} onDragOver={(e) => handleDragOver(e, status)} onDragLeave={() => setDragOverCol(null)} onDrop={() => handleDrop(status)}
              className={`min-w-[260px] sm:min-w-[280px] flex-1 rounded-xl p-3 space-y-2 snap-start transition-colors duration-200 ${isOver ? "bg-primary/10 border-2 border-dashed border-primary/40" : "bg-secondary/30 border-2 border-transparent"}`}
            >
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <span className="ml-auto text-xs bg-secondary px-2 py-0.5 rounded-full text-muted-foreground font-medium">{columnVideos.length}</span>
              </div>
              <AnimatePresence mode="popLayout">
                {columnVideos.map((video) => {
                  const canal = SAMPLE_CANAIS.find((c) => c.id === video.canalId);
                  return (
                    <motion.div key={video.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.15 }}
                      draggable onDragStart={(e) => handleDragStart(e as unknown as React.DragEvent, video.id)} onDragEnd={(e) => handleDragEnd(e as unknown as React.DragEvent)}
                      className={`bg-card border rounded-lg p-3 cursor-grab active:cursor-grabbing transition-all duration-150 select-none group/card ${dragging === video.id ? "border-primary shadow-lg shadow-primary/20 scale-[1.02]" : "border-border hover:border-primary/30"}`}
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {video.notas && <p className="text-xs text-muted-foreground/70 mt-1 truncate italic">{video.notas}</p>}
                        </div>
                        <div className="flex gap-0.5 opacity-0 group-hover/card:opacity-100 transition-opacity shrink-0">
                          <button onClick={(e) => { e.stopPropagation(); handleOpenEdit(video); }} className="p-1 rounded hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors" title="Editar vídeo">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={(e) => { e.stopPropagation(); setDeleteId(video.id); }} className="p-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors" title="Excluir vídeo">
              ...
              {columnVideos.length === 0 && <div className="text-center py-8 text-xs text-muted-foreground/50">Arraste cards aqui</div>}
            </div>
          );
        })}
      </div>
